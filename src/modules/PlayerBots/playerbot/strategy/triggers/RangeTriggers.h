...
namespace ai
{
    ...
    public:
        ...
        virtual bool IsActive() override
        {
            ...
            if (target)
            {
                ...
                if (target->GetTarget() == bot && canMove && target->GetSpeed(MOVE_RUN) > (bot->GetSpeed(MOVE_RUN) * 0.65))
                {
                    return false;
                }

                float const combatReach = (bot->GetCombatReach(false) + target->GetCombatReach(false));
    ...
    public:
        ...
        virtual bool IsActive() override
        {
            ...
            {
                // Don't move if the target is targeting you and you can't add distance between you and the target
                if (target->GetTarget() == bot && !target->IsRooted() && target->GetSpeed(MOVE_RUN) > (bot->GetSpeed(MOVE_RUN) * 0.65))
                {
                    return false;
                }
    ...
    public:
        ...
        virtual bool IsActive() override
        {
            ...
            return target && (bot->GetDistance(target, DIST_CALC_COMBAT_REACH) > (distance - sPlayerbotAIConfig.contactDistance)) || !bot->IsWithinLOSInMap(target, true);
        }
    };
