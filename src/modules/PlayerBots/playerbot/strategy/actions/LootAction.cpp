...
bool LootAction::Execute(Event& event)
{
    ...
    if (!prevLoot.IsEmpty() && prevLoot.guid != lootObject.guid)
    {
        ...
        bot->GetSession()->BotHandleLootReleaseOpcode(packet);
        released = true;
    }
...
bool OpenLootAction::DoLoot(LootObject& lootObject)
{
    ...
    if (creature && creature->HasFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE) && !creature->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE))
    {
        ...
        bot->GetSession()->BotHandleLootOpcode(packet);
...
bool StoreLootAction::Execute(Event& event)
{
    ...
    if (!loot)
    {
        ...
        release << guid;
        bot->GetSession()->BotHandleLootReleaseOpcode(release);
        AI_VALUE(LootObjectStack*, "available loot")->Remove(guid);
        RESET_AI_VALUE(LootObject, "loot target");
        return false;
    }
    ...
    if (gold > 0)
    {
        ...
        bot->GetSession()->BotHandleLootMoneyOpcode(packet);
    }
    ...
    for (uint8 i = 0; i < items; ++i)
    {
        ...
        packet << itemindex;
        bot->GetSession()->BotHandleAutostoreLootItemOpcode(packet);
        ++itemsTaken;
