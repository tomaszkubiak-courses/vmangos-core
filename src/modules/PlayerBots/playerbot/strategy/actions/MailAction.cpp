...
public:
    ...
    bool Process(Player* requester, int index, Mail* mail, PlayerbotAI* ai, Event& event) override
    {
        ...
        if (mail->has_items)
        {
            ...
            {
                Item* item = BotMail(bot)->GetMItem(i->itemGuid);
...
public:
    ...
    bool Process(Player* requester, int index, Mail* mail, PlayerbotAI* ai, Event& event) override
    {
        ...
        if (mail->money)
        {
            ...
            bot->GetSession()->BotHandleMailTakeMoney(packet);
        ...
        else if (mail->has_items)
        {
            ...
            for (std::list<uint32>::iterator i = guids.begin(); i != guids.end(); ++i)
            {
                ...
                packet << *i;
#endif
                Item* item = BotMail(bot)->GetMItem(*i);
                ...
                bot->GetSession()->BotHandleMailTakeItem(packet);
            }
...
#endif
    bot->GetSession()->BotHandleMailDelete(packet);
}
