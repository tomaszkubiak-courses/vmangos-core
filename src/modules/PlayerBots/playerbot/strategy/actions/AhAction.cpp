...
bool AhAction::PostItem(Player* requester, Item* item, uint32 price, Unit* auctioneer, uint32 time)
{
    ...
    bot->GetSession()->BotHandleAuctionSellItem(packet);
    ...
    if (text == "vendor")
    {
        ...
        for (uint32 i = 0; i < checkNumAuctions; i++)
        {
            ...
            case ItemUsage::ITEM_USAGE_AH:
            {
                ...
                bool isWorthBuyingFromAhToResellAtAH = ItemUsageValue::IsWorthBuyingFromAhToResellAtAH(sObjectMgr.GetItemPrototype(auction->itemTemplate), totalCost, auction->GetItemCount());
                pmo.reset();
            ...
            case ItemUsage::ITEM_USAGE_VENDOR:
                ...
                if (totalCost / auction->GetItemCount() >= (int32)sObjectMgr.GetItemPrototype(auction->itemTemplate)->SellPrice)
                    continue;
                power = 1000;
                break;
        ...
        for (auto auctionPower : auctionPowers)
        {
            ...
            std::string reason = ItemUsageValue::ReasonForNeed(usage, auction, auction->GetItemCount(), bot);            
...
bool AhBidAction::BidItem(Player* requester, AuctionEntry* auction, uint32 price, Unit* auctioneer, bool isBuyout, std::string reason)
{
    ...
    uint32 count = auction->GetItemCount();

    ItemPrototype const* proto = sObjectMgr.GetItemPrototype(auction->itemTemplate);

    bot->GetSession()->BotHandleAuctionPlaceBid(packet);
