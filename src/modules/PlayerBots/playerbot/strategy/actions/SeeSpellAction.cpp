...
bool SeeSpellAction::Execute(Event& event)
{
    ...
    {
        // The navmesh area and flags this printed come from cmangos pathfinder extensions
        // (PathFinder::getArea / getFlags) that this core's PathInfo does not have, so the
        // debug line reports the position alone.
        std::ostringstream out;

        if (spellPosition.isOutside())
            out << "[outside]";

        out << " x = " << spellPosition.getX() << ", y = " << spellPosition.getY() << ", z = " << spellPosition.getZ();
    ...
    {
        if (!inRange && selected)
            BotSendPlaySpellVisual(requester->GetSession(), bot->GetObjectGuid(), 6372);
        else if (inRange && !selected)
            BotSendPlaySpellVisual(requester->GetSession(), bot->GetObjectGuid(), 5036);
