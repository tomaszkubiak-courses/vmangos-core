        ...
        if (bot->HasAura(EY_SPELL_NETHERSTORM_FLAG))
        {
            ...
            { //Deliver flag
                ...
                if (bot->IsWithinDist3d(flagDeliverPoint.x, flagDeliverPoint.y, flagDeliverPoint.z, INTERACTION_DISTANCE))
                {
                    ...
                    bot->GetSession()->BotHandleAreaTriggerOpcode(data);
                }
            }
...
bool BGTactics::atFlag(std::vector<BattleBotPath*> const& vPaths, std::vector<uint32> const& vFlagIds)
{
    ...
    for (std::list<ObjectGuid>::iterator i = closeObjects.begin(); i != closeObjects.end(); ++i)
    {
        ...
        if (!sServerFacade.isSpawned(go) || BotGameObjectInUse(go) || go->GetGoState() != GO_STATE_READY)
            continue;
...
#endif
        {
            ...
            //bot->GetSession()->BotHandleGameObjectUseOpcode(data);
            resetObjective();
            return true;
            break;
        }
...
bool BGTactics::useBuff()
{
    ...
    for (std::list<ObjectGuid>::iterator i = closeObjects.begin(); i != closeObjects.end(); ++i)
    {
        ...
        if (sServerFacade.IsDistanceGreaterThan(sServerFacade.GetDistance2d(bot, go), go->GetEntry() == g_buffEntries[0] ? 20.0f : VISIBILITY_DISTANCE_SMALL))
            continue;

        if (needSpeed && go->GetEntry() == g_buffEntries[0])
            foundBuff = true;

        if (needRegen && go->GetEntry() == g_buffEntries[1])
            foundBuff = true;

        // do not move to Berserk buff if bot is healer or has flag
#ifdef MANGOSBOT_ZERO
        if (!(bot->HasAura(BG_WS_SPELL_WARSONG_FLAG) || bot->HasAura(BG_WS_SPELL_SILVERWING_FLAG)) && !ai->IsHeal(bot) && go->GetEntry() == g_buffEntries[2])
#else
        if (!(bot->HasAura(BG_WS_SPELL_WARSONG_FLAG) || bot->HasAura(BG_WS_SPELL_SILVERWING_FLAG) || bot->HasAura(EY_SPELL_NETHERSTORM_FLAG)) && !ai->IsHeal(bot) && go->GetEntry() == g_buffEntries[2])
#endif
            foundBuff = true;
