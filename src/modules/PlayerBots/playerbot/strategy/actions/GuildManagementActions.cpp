...
bool GuildLeaveAction::Execute(Event& event)
{
    ...
    if (guild->GetMemberSize() > sPlayerbotAIConfig.guildMaxBotLimit)
    {
        ...
        guild->BroadcastToGuild(
            bot->GetSession(),
            BOT_TEXT2("I am leaving this guild to prevent it from reaching the %guild_bot_limit member limit.", placeholders).c_str(),
            LANG_UNIVERSAL
        );
    }
    ...
    WorldPacket packet;
    bot->GetSession()->BotHandleGuildLeaveOpcode(packet);
    return true;
}
