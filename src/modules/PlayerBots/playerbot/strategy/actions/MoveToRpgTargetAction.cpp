...
bool MoveToRpgTargetAction::Execute(Event& event)
{
    ...
    if (unit && (!unit->IsStopped()) && !urand(0, 20) && guidP.sqDistance2d(bot) < INTERACTION_DISTANCE * INTERACTION_DISTANCE * 2)
    {
        AI_VALUE(std::set<ObjectGuid>&,"ignore rpg target").insert(AI_VALUE(GuidPosition, "rpg target"));
    ...
    if (unit && (!unit->IsStopped()) && bot->GetDistance(unit) < INTERACTION_DISTANCE * 2 && unit->GetMotionMaster()->GetCurrentMovementGeneratorType() != IDLE_MOTION_TYPE)
    {
...
bool MoveToRpgTargetAction::isUseful()
{
    ...
    if(MEM_AI_VALUE(WorldPosition, "current position")->LastChangeDelay() < 60)
        if ((!bot->IsStopped()) && bot->GetMotionMaster() && bot->GetMotionMaster()->GetCurrentMovementGeneratorType() != FOLLOW_MOTION_TYPE)
            return false;
