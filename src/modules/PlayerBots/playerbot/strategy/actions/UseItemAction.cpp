    ...
    for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch (m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_OPEN_LOCK:
            {
                ...
                // CanOpenLock reports the skill it wants through out parameters here, rather
                // than filling a per-effect record on the spell.
...
bool UseAction::Execute(Event& event)
{
    ...
    else
    {
        ...
        if (!items.empty())
        {
            ...
            if (items.size() > 1)
            {
                targetItem = BotGetItemByEntry(bot, items[1]);
            }
        }
...
bool UseAction::UseGameObject(Player* requester, Event& event, GameObject* gameObject)
{
    ...
    if (!sServerFacade.isSpawned(gameObject) || BotGameObjectInUse(gameObject) || gameObject->GetGoState() != GO_STATE_READY)
    {
