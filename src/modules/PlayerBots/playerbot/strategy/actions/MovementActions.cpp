        ...
        if (!targets.empty() && movePosition)
        {
            ...
            for (auto p : points)
            {
                ...
                for (auto target : targets)
                {
                    ...
                    float range = BotGetAttackDistance(unit, bot);
...
bool MovementAction::Follow(Unit* target, float distance, float angle)
{
    ...
    if (tDist > sPlayerbotAIConfig.sightDistance || (target->IsFlying() && !BotIsFreeFlying(bot)) || target->IsTaxiFlying())
    {
...
bool SetBehindTargetAction::Execute(Event& event)
{
    ...
    float distance = (bot->GetCombatReach(true) + target->GetCombatReach(true)) * 0.8f;
...
#ifdef MANGOSBOT_TWO
    target->GetMap()->GetLosHitPosition(ox, oy, oz + bot->GetCollisionHeight(), x, y, z, bot->GetPhaseMask(), -0.5f);
#else
...
#ifdef MANGOSBOT_TWO
        if (jumper->GetMap()->GetLosHitPosition(ox, oy, oz, fx, fy, fz, jumper->GetPhaseMask(), -0.5f))
#else
...
#ifdef MANGOSBOT_TWO
        foundCollision = jumper->GetMap()->GetLosHitPosition(ox, oy, oz, fx, fy, fz, jumper->GetPhaseMask(), -0.5f);
#else
...
#ifdef MANGOSBOT_TWO
                jumper->GetMap()->GetLosHitPosition(fx, fy, fz, fx, fy, fz_mod, jumper->GetPhaseMask(), -0.5f);
#else
