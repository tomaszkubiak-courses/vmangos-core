...
bool UpdateStrategyDependenciesAction::isUseful()
{
    if (!strategiesToUpdate.empty())
    {
        ...
        for (const StrategyToUpdate& strategy : strategiesToUpdate)
        {
            ...
            for (const std::vector<std::string>& strategyRequiredAliases : strategy.strategiesRequired)
            {
...
bool SetPetAction::Execute(Event& event)
{
    ...
    if (pet)
    {
        ...
        else if (command == "aggressive")
        {
            ...
            packet << data;
            packet << uint64(0);
            bot->GetSession()->BotHandlePetAction(packet);
        ...
        else if (command == "abandon")
        {
            if (bot->GetClass() == CLASS_HUNTER)
            {
                ...
                bot->GetSession()->BotHandlePetAbandon(packet);
