    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); ++id)
    {
        ...
        if (skillName.empty() || skillIds.find(id) != skillIds.end() || (loc < MAX_LOCALE && skillIds.empty()))
        {
            if (unlearn)
            {
                ...
                // Whether a skill can be dropped is a property of SkillRaceClassInfo.dbc, read
                // for this character's race and class.
                SkillRaceClassInfoEntry const* skillInfo = GetSkillRaceClassInfo(id, bot->GetRace(), bot->GetClass());
                if (!skillInfo || !(skillInfo->flags & SKILL_FLAG_CAN_UNLEARN))
                {
                    ai->TellPlayerNoFacing(requester, BOT_TEXT2("Unable to unlearn %skillname", args));
                    return false;
                }

                // Dropping a skill is setting it to zero here; there is no separate step call.
