...
bool BGJoinAction::shouldJoinBg(BattleGroundQueueTypeId queueTypeId, BattleGroundBracketId bracketId)
{
    ...
    bool noLag = sWorld.GetCurrentDiff() < (sRandomPlayerbotMgr.GetPlayers().empty() ? sPlayerbotAIConfig.diffEmpty : sPlayerbotAIConfig.diffWithPlayer) * 1.1;
...
// in wotlk only arena requires battlemaster guid
   ...
   bot->GetSession()->BotHandleBattlemasterJoinOpcode(packet);
   return true;
}
...
bool FreeBGJoinAction::shouldJoinBg(BattleGroundQueueTypeId queueTypeId, BattleGroundBracketId bracketId)
{
    ...
    bool isRated = false;
    bool hasTeam = false;
    bool noLag = sWorld.GetCurrentDiff() < (sRandomPlayerbotMgr.GetPlayers().empty() ? sPlayerbotAIConfig.diffEmpty : sPlayerbotAIConfig.diffWithPlayer) * 1.5;
...
bool BGLeaveAction::Execute(Event& event)
{
    ...
    if (bot->InBattleGround())
    {
        ...
        bot->GetSession()->BotHandleLeaveBattlefieldOpcode(leave);
    ...
    {
        bot->GetSession()->BotHandleLeaveBattlefieldOpcode(packet);
    }
    else
...
bool BGStatusCheckAction::Execute(Event& event)
{
    ...
    bot->GetSession()->BotHandleBattlefieldStatusOpcode(packet);
