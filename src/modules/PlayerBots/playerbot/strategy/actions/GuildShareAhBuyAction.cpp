...
uint32 GuildShareAhBuyAction::CountMailboxItems(uint32 itemId)
{
    ...
    for (PlayerMails::iterator itr = BotMail(bot)->GetMailBegin(); itr != BotMail(bot)->GetMailEnd(); ++itr)
    {
        Mail* mail = *itr;
        ...
        for (MailItemInfoVec::const_iterator itemItr = mail->items.begin(); itemItr != mail->items.end(); ++itemItr)
        {
            ...
            {
                Item* mailItem = BotMail(bot)->GetMItem(itemItr->itemGuid);
...
bool GuildShareAhBuyAction::Execute(Event& event)
{
    ...
    if (bestCandidate.auction)
    {
        ...
        // No AuctionEntry::UpdateBid here: a purchase goes through the auction handler, the
        // same way AhAction places a bid, so the money, the mails and the auction's removal
        // are all done by the core.
        WorldPacket packet;
