...
{
    WorldPacket p;
    bot->GetSession()->BotHandleQueryNextMailTime(p);   
    ...
    for (PlayerMails::iterator i = BotMail(bot)->GetMailBegin(); i != BotMail(bot)->GetMailEnd(); ++i)
    {
        mails.push_back(*i);
    }
...
{
    if (ai->GetMaster() || !BotMail(bot)->GetMailSize() || bot->InBattleGround())
        return false;
...
void CheckMailAction::ProcessMail(Mail* mail, Player* owner)
{
    ...
    for (MailItemInfoVec::iterator i = mail->items.begin(); i != mail->items.end(); ++i)
    {
        ...
        draft.AddItem(item);
        BotMail(bot)->RemoveMItem(i->itemGuid);
