...
bool TradeAction::Execute(Event& event)
{
    ...
    if (!bot->GetTrader())
    {
        ...
        if (!player->GetTrader())
        {
            ...
            bot->GetSession()->BotHandleInitiateTradeOpcode(packet);
            return true;
        }
    ...
    if (copper > 0)
    {
        ...
        packet << copper;
        bot->GetSession()->BotHandleSetTradeGoldOpcode(packet);
    }
...
bool TradeAction::TradeItem(const Item& item, int8 slot)
{
    ...
    else
    {
        for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT && tradeSlot == -1; i++)
        {
            if (pTrade->GetItem(TradeSlots(i)) == itemPtr)
            {
                ...
                bot->GetSession()->BotHandleClearTradeItemOpcode(packet);
    ...
    packet << (uint8) tradeSlot << (uint8) item.GetBagSlot()
        << (uint8) item.GetSlot();
    bot->GetSession()->BotHandleSetTradeItemOpcode(packet);
    return true;
}
