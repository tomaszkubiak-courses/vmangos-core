...
uint32 LootAccess::GetLootStatusFor(Player const* player) const
{
	...
	{
		// This core's GetSlotTypeForSharedLoot also wants the object being looted. The status
		// is only read as "is anything left here", so the permission stays NONE.
		LootSlotType slotType = lootItem.GetSlotTypeForSharedLoot(NONE_PERMISSION, const_cast<Player*>(player), loot->GetLootTarget());
