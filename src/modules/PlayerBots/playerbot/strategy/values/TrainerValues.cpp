    ...
    for (auto& [templateOrEntryId, trainers] : trainerTemplateIds)
    {
        ...
        for (auto& [id, trainerSpell] : trainer_spells->spellList)
        {
            ...
            for (auto& [otherTrainerSpell, trainers] : (*spellMap)[trainerType][spellRequirement])
            {
                ...
                if (GetTrainerLearnedSpell(otherTrainerSpell) != GetTrainerLearnedSpell(&trainerSpell))
#else
            ...
            if (trainerType == TRAINER_TYPE_TRADESKILLS)
            {
                ...
                else
                {
                    ...
                    SpellEntry const* spell = sSpellTemplate.LookupEntry<SpellEntry>(GetTrainerLearnedSpell(&trainerSpell));
#else
...
#ifdef MANGOSBOT_ZERO
                if (bot->GetLevel() < 10 && sSpellMgr.IsProfessionSpell(GetTrainerLearnedSpell(trainerSpell)) && sSpellMgr.GetSpellRank(GetTrainerLearnedSpell(trainerSpell)) == 1)
#else
