...
, loginProgressBar(NULL)
{
    ...
    {
        // PlayerbotCommandServer, an external TCP port for driving bots from outside the
        // game, was dropped with the Boost dependency - it was the only user of
        // boost::asio. AiPlayerbot.CommandServerPort is inert as a result.
        ...
        try
        {
            ...
            if (sPlayerbotAIConfig.randomBotAutologin)
            {
                ForEachPlayerbot([&](Player* bot) {
                    ...
                    out << bot->GetLevel() << ",";
                    out << bot->GetHealth() << ",";
                    out << bot->GetPowerPercent(POWER_MANA) << ",";
                    out << bot->GetMoney() << ",";
...
void RandomPlayerbotMgr::LoginFreeBots()
{
    if (!sPlayerbotAIConfig.freeAltBots.empty() && sPlayerbotAIConfig.botAutologin != BotAutoLogin::LOGIN_ONLY_ALWAYS_ACTIVE)
    {
        ...
        for (auto [accountId, botGuid] : sPlayerbotAIConfig.freeAltBots)
        {
            ...
            Player* bot = ObjectAccessor::FindPlayerNotInWorld(guid);
...
void RandomPlayerbotMgr::CheckLfgQueue()
{
    ...
    for (auto i : players)
    {
        ...
        if (group)
        {
            ...
            {
                isLFG = true;
                if (LFGGroupQueueInfo const* lfgInfo = sWorld.GetLFGQueue().GetGroupQueueInfo(group->GetId()))
                {
        ...
        else
        {
            ...
            {
                isLFG = true;
                if (LFGPlayerQueueInfo const* lfgInfo = sWorld.GetLFGQueue().GetPlayerQueueInfo(player->GetObjectGuid()))
                {
                    uint32 lfgType = (zoneId << 16) | lfgInfo->areaId;
                    LfgDungeons[player->GetTeam()].push_back(lfgType);
                }
...
void RandomPlayerbotMgr::RandomTeleport(Player* bot, std::vector<WorldLocation> &locs, bool hearth, bool activeOnly)
{
    ...
    if (sPlayerbotAIConfig.randomBotTeleportNearPlayer && activeOnly)
    {
        tlocs.erase(std::remove_if(tlocs.begin(), tlocs.end(), [this](const WorldPosition& l)
        {
            ...
            if (tMap && tMap->IsContinent() && HasRealPlayerOnMap(mapId))
            {
                uint32 zoneId = sTerrainMgr.GetZoneId(mapId, l.x, l.y, l.z);
                if (HasRealPlayerInZone(mapId, zoneId))
                {
        ...
        /*if (!tlocs.empty())
        {
            tlocs.erase(std::remove_if(tlocs.begin(), tlocs.end(), [bot](const WorldPosition& l)
            {
                ...
                if (area)
                {
                    if (!HasRealPlayerInZone(mapId, area->ZoneId ? area->ZoneId : area->Id))
                        return true;
                }
            }), tlocs.end());
        }*/
    ...
    for (int i = 0; i < tlocs.size(); i++)
    {
        for (int attemtps = 0; attemtps < 3; ++attemtps)
        {
            ...
            // locale array - Name was dereferencing it to a single char and
...
void RandomPlayerbotMgr::HandleCommand(uint32 type, const std::string& text, Player& fromPlayer, std::string channelName, Team team, uint32 lang, const std::string& to)
{
    ForEachPlayerbot([&](Player* bot)
    {
        ...
        if (!channelName.empty())
        {
            ...
            {
                Channel* chn = cMgr->GetChannel(channelName, BotPlayerPointer(bot));
                if (!chn)
                {
                    return;
                }
...
void RandomPlayerbotMgr::PrintStats(uint32 requesterGuid)
{
    ...
    if (requester) { requester->SendSysMessage(ss.str().c_str()); }
    ...
    ForEachPlayerbot([this, &dps, &heal, &tank, &active, &update, &randomize, &teleport, &changeStrategy, &dead, &combat, &revive, &taxi, &moving, &mounted, &afk, &alliance, &horde, &perRace, &perClass, &stateCount, &questCount](Player* bot)
    {
        ...
        if ((!bot->IsStopped()) && !bot->IsTaxiFlying() && !bot->IsFlying())
            moving++;
...
bool RandomPlayerbotMgr::HasRealPlayerInZone(uint32 mapId, uint32 zoneId)
{
...
bool RandomPlayerbotMgr::HasRealPlayerOnMap(uint32 mapId)
{
    for (auto const& entry : players)
    {
        Player* player = entry.second;
...
void RandomPlayerbotMgr::MirrorAh()
{
    ...
    for (uint32 i = 0; i < sAuctionHouseStore.GetNumRows(); ++i)
    {
        ...
        // This runs off the world thread, so the entries cannot be held onto: take a
        // snapshot, which copies them out under the auction house lock.
        for (AuctionSnapshot const& snap : auctionHouse->GetAuctionsSnapshot())
        {
            if (!snap.buyout)
                continue;
            ...
            ahMirror[snap.itemTemplate].push_back(snap);
        }
    }
