...
void PlayerbotFactory::InitPet()
{
    ...
    if (!pet)
    {
		...
		for (int i = 0; i < 100; i++)
		{
            ...
            // No owner-talent auras on pets in this core: the talents that buff a pet are
            // applied by the aura system when the pet is summoned, not cast onto it here.
            pet->UpdateAllStats();
            bot->SetPet(pet);
...
void PlayerbotFactory::InitTradeSkills()
{
    ...
    for (uint32 id = 0; id < sCreatureStorage.GetMaxEntry(); ++id)
    {
        ...
        for (TrainerSpellMap::const_iterator itr = trainer_spells->spellList.begin(); itr != trainer_spells->spellList.end(); ++itr)
        {
...
#ifdef MANGOSBOT_ZERO
            if (GetTrainerLearnedSpell(tSpell))
            {
                bool learned = false;
                for (int j = 0; j < 3; ++j)
                {
                ...
                if (!learned) bot->LearnSpell(GetTrainerLearnedSpell(tSpell), false);
            }
            else
            ...
            {
                for (auto learnSpell : GetTrainerLearnedSpell(tSpell))
                {
                    bool learned = false;
                    for (int j = 0; j < 3; ++j)
                    {
...
void PlayerbotFactory::InitAvailableSpells()
{
    ...
    // The class spells a character starts each level with are what LearnDefaultSpells
    // above already grants here; anything past that is bought from a trainer, which the
    // bot does through AutoLearnSpellAction.
...
void PlayerbotFactory::AddPrevQuests(uint32 questId, std::list<uint32>& questIds)
{
    ...
    for (auto iter = quest->prevQuests.begin(); iter != quest->prevQuests.end(); ++iter)
    {
