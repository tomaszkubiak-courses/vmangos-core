...
std::string ChatHelper::formatWorldEntry(int32 entry)
{
    ...
    if (loc_idx >= 0 && entry > 0)
    {
        name = BotLocalizedCreatureName(entry, loc_idx, name);
    }
...
std::string ChatHelper::formatItem(ItemQualifier& itemQualifier, int count, int total)
{
    ...
    if (loc_idx >= 0)
    {
        name = BotLocalizedItemName(itemQualifier.GetId(), loc_idx, name);
    }
...
{
    if (botstr::iequals(text, "healer"))
        return BotRoles::BOT_ROLE_HEALER;
    else if (botstr::iequals(text, "tank"))
        return BotRoles::BOT_ROLE_TANK;
    else if (botstr::iequals(text, "dps"))
...
{
    if (botstr::iequals(text, "male"))
        return GENDER_MALE;
    else if (botstr::iequals(text, "female"))
...
{
    if (botstr::iequals(text, "alliance"))
        return ALLIANCE;
    else if (botstr::iequals(text, "horde"))
        return HORDE;
    else if (Qualified::isValidNumberString(text))
    {
...
uint32 ChatHelper::parseClass(const std::string& text)
{
    for (auto& [classId, className] : classes)
        if (botstr::iequals(className, text))
            return classId;
...
uint32 ChatHelper::parseRace(const std::string& text)
{
    for (auto& [raceId, raceName] : races)
        if (botstr::iequals(raceName, text))
            return raceId;
