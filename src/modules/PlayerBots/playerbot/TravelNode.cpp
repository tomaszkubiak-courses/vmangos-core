...
void TravelPath::ClipPath(PlayerbotAI* ai, Unit* mover, bool ignoreEnemyTargets)
{
    ...
    for (auto p = fullPath.begin(); p != fullPath.end(); p++)
    {
        for (auto& targetGuid : targets)
        {
            ...
            float range = BotGetAttackDistance(unit, mover);
...
void TravelNodeMap::LoadMaps()
{
    ...
    // the VMaNGOS equivalent and hands back bare file names, which is what the digit
    // arithmetic below wants anyway. Read once rather than once per map: the listing does
    // not change between passes, and there are several hundred maps.
    std::vector<std::string> const mmapFiles = IO::Filesystem::GetAllFilesInFolder(
        sWorld.GetDataPath() + "mmaps", IO::Filesystem::OutputFilePath::JustFileName);

    char const mmapExtension[] = ".mmtile";
    size_t const mmapExtensionLength = sizeof(mmapExtension) - 1;
    size_t const mmapNameLength = 7; // mmmxxyy
    ...
    for (uint32 i = 0; i < sMapStore.GetNumRows(); ++i)
    {
        ...
        {
            if (fileName.size() != mmapNameLength + mmapExtensionLength ||
                fileName.compare(mmapNameLength, mmapExtensionLength, mmapExtension) != 0)
                continue;

            char const* fileNameString = fileName.c_str();
    ...
    {        
        ...
        if (data && (data->type == GAMEOBJECT_TYPE_TRANSPORT || data->type == GAMEOBJECT_TYPE_MO_TRANSPORT))
        {
            ...
            else //Boats/Zepelins
            {
                ...
                // Path<> here is indexable but not iterable, and its nodes are entries
                // rather than pointers.
                for (size_t i = 0; i < path.size(); ++i)
                {
                    TaxiPathNodeEntry const* p = &path[i];
...
void TravelNodeMap::generateTaxiPaths()
{
    for (uint32 i = 0; i < sTaxiPathStore.GetNumRows(); ++i)
    {
        ...
        if (startNode->fDist(WorldPosition(firstNode.mapid, firstNode.x, firstNode.y, firstNode.z, 0.0)) > 0.1f)
            ppath.push_back(*startNode->getPosition());

        for (size_t i = 0; i < nodes.size(); ++i)
        {
            TaxiPathNodeEntry const& n = nodes[i];
