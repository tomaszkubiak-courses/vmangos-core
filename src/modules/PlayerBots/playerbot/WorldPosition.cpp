...
float WorldPosition::projectOnSegment(const WorldPosition& p1, const WorldPosition& p2) const
{
    ...
    return ((this->x - p1.x) * dx + (this->y - p1.y) * dy + (this->z - p1.z) * dz) / lenSq;
}

G3D::Vector3 WorldPosition::getVector3() const
{
    return G3D::Vector3(this->x, this->y, this->z); 
}
...
{
    transport->CalculatePassengerPosition(this->x, this->y, this->z, &this->o);
}

void WorldPosition::CalculatePassengerOffset(GenericTransport* transport)
{
    transport->CalculatePassengerOffset(this->x, this->y, this->z, &this->o);
}
...
bool WorldPosition::isOnTransport(GenericTransport* transport)
{
    ...
    bool result0 = VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(this->mapId, this->x, this->y, this->z + 0.5f, below.getX(), below.getY(), below.getZ(), below.x, below.y, below.z, 0.0f);
...
bool WorldPosition::SetOnTransport(GenericTransport* transport, int32 startHeight, int32 endHeight)
{
    ...
    bool result = VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(this->mapId, this->x, this->y, this->z, below.getX(), below.getY(), below.getZ(), below.x, below.y, below.z, 0.0f);
...
{
    MANGOS_ASSERT(std::isfinite(this->x) && std::isfinite(this->y) && std::isfinite(this->z));
    ...
    float curPoint[VERTEX_SIZE] = {this->y, this->z, this->x };
...
{
    float groundZ = getMap(getFirstInstanceId())->GetHeight(this->x, this->y, this->z+0.5f, true), maxZ;
    maxZ = getTerrain()->GetWaterOrGroundLevel(this->x, this->y, this->z + 0.5f, &groundZ, true);
...
uint32 WorldPosition::getUnitsAggro(const std::list<ObjectGuid>& units, const Player* bot) const
{
    ...
    for (auto guid : units)
    {
        ...
        if (this->sqDistance(unit) > BotGetAttackDistance(unit, bot) * BotGetAttackDistance(unit, bot))
            continue;
