...
bool PlayerbotAIConfig::Initialize()
{
    ...
    if (botConfigFile.empty() || !config.LoadFromFile(botConfigFile))
    {
    ...
    // Share of the random bots of each faction placed in one of that faction's three
    // capitals at login instead of at a grind location, split evenly over the three.
    // Custom behaviour, so off unless asked for.
    citySpawnPercent = config.GetIntDefault("AiPlayerbot.CitySpawnPercent", 0);
    if (citySpawnPercent > 100)
        citySpawnPercent = 100;
    ...
    {
        levelProbability[level] = config.GetIntDefault(std::string("AiPlayerbot.LevelProbability." + std::to_string(level)).c_str(), 100);
    }
    ...
    for (uint32 race = 1; race < MAX_RACES; ++race)
    {
        ...
        if (race > 0)
        {
            int rProb = config.GetIntDefault(std::string("AiPlayerbot.ClassRaceProb.0." + std::to_string(race)).c_str(), 100);
    ...
    for (uint32 cls = 1; cls < MAX_CLASSES; ++cls)
    {
        int cProb = config.GetIntDefault(std::string("AiPlayerbot.ClassRaceProb." + std::to_string(cls)).c_str(), -1);
    ...
    for (uint32 race = 1; race < MAX_RACES; ++race)
    {
        for (uint32 cls = 1; cls < MAX_CLASSES; ++cls)
        {
            int rcProb = config.GetIntDefault(std::string("AiPlayerbot.ClassRaceProb." + std::to_string(cls) + "." + std::to_string(race)).c_str(), -1);
            if (rcProb >= 0)
    ...
    if (useFixedClassRaceCounts)
    {
            ...
            for (uint32 cls = 1; cls < MAX_CLASSES; ++cls)
	    {
		    ...
		    int count = config.GetIntDefault(std::string(key).c_str(), -1);
