...
void PlayerbotHolder::AddPlayerBot(uint32 guidLow, uint32 masterAccountId)
{
    ...
    Player* existing = ObjectAccessor::FindPlayerNotInWorld(botGuid);
    ...
    if (ghost && !ghost->IsInWorld())
    {
        ...
        if (ghost->IsBeingTeleportedFar())
        {
            ...
            ghost->GetSession()->BotHandleMoveWorldportAckOpcode(BotEmptyPacket(MSG_MOVE_WORLDPORT_ACK));
        }
        else if (ghost->IsBeingTeleportedNear())
        {
            ...
            p << uint32(time(0));
            ghost->GetSession()->BotHandleMoveTeleportAckOpcode(p);
        }
...
void PlayerbotHolder::HandlePlayerBotLoginCallback(std::unique_ptr<QueryResult> /*dummy*/, SqlQueryHolder* holder)
{
    ...
    if (ObjectAccessor::FindPlayerNotInWorld(lqh->GetGuid()))
    {
        delete holder;
        return;
    }
    ...
    // ever firing.
    // A socketless session is tagged "<BOT>" by WorldSession's own constructor, which is
    // what PlayerbotAI::IsRealPlayer() reads.
    WorldSession* botSession = new WorldSession(lqh->GetAccountId(), /*sock*/ nullptr, SEC_PLAYER,
                                                /*mute_time*/ 0, LOCALE_enUS);
    // Marks the session as one this module drives: it has no socket, and without this
    // the core would never let its queued packets through.
    botSession->SetPlayerbotSession(true);
...
void PlayerbotHolder::UpdateSessions(uint32 elapsed)
{
    ForEachPlayerbot([&](Player* bot)
    {
        ...
        {
            bot->GetSession()->ProcessQueuedPacketsNow();
        }
        else
        {
...
void PlayerbotHolder::JoinChatChannels(Player* bot)
{
    ...
    if (sRandomPlayerbotMgr.IsFreeBot(bot))
    {
        ...
        pkt << ""; // Pass
        bot->GetSession()->BotHandleJoinChannelOpcode(pkt);
    }
...
void PlayerbotHolder::OnBotLogin(Player * const bot)
{
    ...
    if (group)
    {
        ...
        if (!groupValid)
        {
            ...
            bot->GetSession()->BotHandleGroupDisbandOpcode(p);
        }
    }
...
void PlayerbotMgr::CheckTellErrors(uint32 elapsed)
{
    ...
    for (PlayerBotErrorMap::iterator i = errors.begin(); i != errors.end(); ++i)
    {
        ...
        ChatHandler(master->GetSession()).PSendSysMessage("%s", ChatHelper::stripUnsupportedLinks(out.str()).c_str());
    }
    errors.clear();
}
...
std::string PlayerbotHolder::HandleBotAlways(Player* bot, Player* master, const std::string param)
{
    ...
    if (always == BotAlwaysOnline::DISABLED || always == BotAlwaysOnline::DISABLED_BY_COMMAND)
    {
        ...
        {
            Player* player = ObjectAccessor::FindPlayerNotInWorld(guid);
            if (player)
                OnBotLogin(player);
        }
    ...
    else
    {
        ...
        Player* onlineBot = ObjectAccessor::FindPlayerNotInWorld(guid);
