...
void PlayerbotAI::UpdateAI(uint32 elapsed, bool minimal)
{
    ...
    if (botMoving && !bot->IsBeingTeleported() && bot->IsInWorld())
    {
        ...
        if (lootObject)
        {
            ...
            if (Loot* loot = sLootMgr.GetLoot(bot, bot->GetLootGuid()))
            {
                ...
                if (shouldRelease)
                {
                    // Loot has no Release() here; the release runs through the handler, the
                    // same path a client's CMSG_LOOT_RELEASE takes.
                    WorldPacket releasePacket(CMSG_LOOT_RELEASE, 8);
                    releasePacket << bot->GetLootGuid();
                    bot->GetSession()->BotHandleLootReleaseOpcode(releasePacket);
                }
            }
        }
    }
    ...
    if (jumpTime && (jumpTime < curTime || (jumpTime + 10000 < curTime)))
    {
        ...
        else
        {
            ...
            // MotionMaster::MoveFall is a cmangos generator; this core lets the fall run on
            // its own movement flags, so there is nothing to start here.
...
void PlayerbotAI::UpdateAIInternal(uint32 elapsed, bool minimal)
{
    ...
    if (bot->GetSession()->IsLogingOut())
    {
        ...
        // Was a WorldSession::GetState() != WORLD_SESSION_STATE_READY test. This core keeps
        // no session lifecycle enum, so ask what that test was really after: whether the
        // owner is still here to command the bot.
        if (!master || !master->IsInWorld() || master->GetSession()->IsLogingOut())
            logout = true;
...
void PlayerbotAI::HandleTeleportAck()
{
	...
	if (bot->IsBeingTeleportedNear())
	{
       ...
       /* WorldLocation dest = bot->GetTeleportDest();
        ...
        bot->GetSession()->BotHandleMoveTeleportAckOpcode(p);
...
#endif
        {
            ...
            if (guid1 != bot->GetObjectGuid()) // do not reply to self
            {
                ...
                if (!isAiChat || isFromFreeBot)
                {
                    ...
                    if (botstr::istarts_with(message, sPlayerbotAIConfig.toxicLinksPrefix)
...
#ifndef MANGOSBOT_ZERO
    if (bot->IsFlying() && !BotIsFreeFlying(bot))
    {
...
void PlayerbotAI::DropQuest(uint32 questIdToDrop)
{
    for (uint16 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot)
    {
        ...
        if (questId == questIdToDrop)
        {
            ...
            bot->TakeOrReplaceQuestStartItems(questId, false, false);
...
ChatChannelSource PlayerbotAI::GetChatChannelSource(Player* bot, uint32 type, std::string channelName)
{
    if (type == CHAT_MSG_CHANNEL)
    {
        ...
        else
        {
            ...
            const Channel* channel = cMgr->GetChannel(channelName, BotPlayerPointer(bot));
            ...
            for (auto& player : sRandomPlayerbotMgr.GetPlayers())
            {
                if (player.second->GetGuildId() == bot->GetGuildId())
                {
                        ...
                        sPlayerbotAIConfig.llmBotToBotChatChance))
                    {
                        ...
                        BotQueuePacket<WorldPackets::Chat::ChatMessage>(bot->GetSession(), packet_template);
                        return true;
                    }
                    break;
                }
...
bool PlayerbotAI::SayToWorld(std::string msg)
{
    ...
    Channel* worldChannel = cMgr->GetJoinChannel("World");
    if (!worldChannel)
        return false;
...
bool PlayerbotAI::SayToGeneral(std::string msg)
{
    ...
    for (auto const& [key, channel] : cMgr->GetChannels())
    {
        ...
        if (channel && channel->GetChannelId() == ChatChannelId::GENERAL
            && botstr::contains(channel->GetName(), GetLocalizedAreaName(current_zone)))
        {
...
bool PlayerbotAI::SayToTrade(std::string msg)
{
    ...
    for (auto const& [key, channel] : cMgr->GetChannels())
    {
        if (channel && channel->GetChannelId() == ChatChannelId::TRADE
            && botstr::contains(channel->GetName(), GetLocalizedAreaName(GetAreaEntryByAreaID(ImportantAreaId::CITY))))
        {
            channel->Say(bot->GetObjectGuid(), msg.c_str(), LANG_UNIVERSAL);
            return true;
        }
...
bool PlayerbotAI::GetSpellRange(std::string name, float* maxRange, float* minRange)
{
    ...
    if(spellId)
    {
        ...
        if (spellInfo)
        {
            ...
            if(spellRangeEntry)
            {
                // SpellRange.dbc's flags column is not loaded by this core, so the melee /
                // ranged distinction comes from the range index instead: index 2 is the
                // dynamic melee combat range (SPELL_RANGE_IDX_COMBAT).
                float spellMinRange = 0.0f, spellMaxRange = 0.0f, rangeMod = 0.0f;
                if (spellInfo->rangeIndex == SPELL_RANGE_IDX_COMBAT)
                {
                    rangeMod = bot->GetCombatReach(true) * 2.0f;
                }
                else
                {
...
bool PlayerbotAI::CanCastSpell(uint32 spellid, Unit* target, uint8 effectMask, bool checkHasSpell, Item* itemTarget, bool ignoreRange, bool ignoreInCombat, bool ignoreMount, SpellCastResult* checkResult)
{
    ...
    Pet* pet = bot->GetPet();
    if (pet && pet->HasSpell(spellid) && pet->IsSpellReady(sSpellMgr.GetSpellEntry(spellid)))
    {
        if (checkResult)
        {
    ...
    if (!bot->IsSpellReady(sSpellMgr.GetSpellEntry(spellid)))
    {
        if (checkResult)
        {
...
//#ifdef MANGOSBOT_TWO
//        bot->GetMap()->GetLosHitPosition(ox, oy, oz + max_height, fx, fy, fz, bot->GetPhaseMask(), -0.5f);
//#else
//        bot->GetMap()->GetLosHitPosition(ox, oy, oz + 2.0f, fx, fy, fz, -0.5f);
//#endif
...
bool PlayerbotAI::CastPetSpell(uint32 spellId, Unit* target)
{
    ...
    if (pet && spellId && pet->HasSpell(spellId))
    {
        ...
        bot->GetSession()->BotHandlePetAction(data);
        return true;
    }
...
bool PlayerbotAI::CanCastVehicleSpell(uint32 spellId, Unit* target)
{
...
#endif
#ifdef CMANGOS
    if (!vehicle->IsSpellReady(sSpellMgr.GetSpellEntry(spellId)))
        return false;
#endif
    ...
    if (CastingTime && (!vehicle->IsStopped()))
        return false;
    ...
    if (siegePos.isSet())
        ...
        dest = spellTarget->GetPosition().WithMapId(spellTarget->GetMapId());
...
bool PlayerbotAI::ChannelHasRealPlayer(std::string channelName)
{
    if (ChannelMgr* cMgr = channelMgr(bot->GetTeam()))
    {
        ...
        if (Channel* chn = cMgr->GetChannel(channelName, BotPlayerPointer(bot)))
        {
...
ActivePiorityType PlayerbotAI::GetPriorityType()
{
    ...
    // Which maps and zones hold a real player is tracked by the module, not by the map -
    // see RandomPlayerbotMgr::HasRealPlayerInZone.
    if (bot->IsBeingTeleported() || !bot->IsInWorld() || !sRandomPlayerbotMgr.HasRealPlayerOnMap(bot->GetMapId()))
        return ActivePiorityType::IN_INACTIVE_MAP;

    if (!sRandomPlayerbotMgr.HasRealPlayerInZone(bot->GetMapId(), bot->GetZoneId()))
...
bool PlayerbotAI::HasQuestItemsInWOLootList(WorldObject* wo)
{
    ...
    // Loot here exposes its items directly; LootAccess picks the ones this bot may take.
    if (woLoot)
        for (LootItem* lootItem : LootAccess(woLoot).GetLootContentFor(bot))
            lootItemList.push_back(*lootItem);
...
bool PlayerbotAI::CanLootSomethingFromWO(WorldObject* wo)
{
    ...
    if (guid.IsCreature())
    {
        ...
        if (creature && sServerFacade.GetDeathState(creature) == CORPSE)
        {
            ...
            for (LootItem* lootItem : LootAccess(&creature->loot).GetLootContentFor(bot))
                lootItemList.push_back(*lootItem);
    ...
    else if (wo->IsGameObject())
    {
        ...
        if (go)
        {
            ...
            for (LootItem* lootItem : LootAccess(&go->loot).GetLootContentFor(bot))
                lootItemList.push_back(*lootItem);
...
void PlayerbotAI::SendDelayedPacket(WorldSession* session, futurePackets futPackets)
{
    std::thread t([session, futPacket = std::move(futPackets)]() mutable {
        for (auto& delayedPacket : futPacket.get())
        {
            ...
            // Everything sent this way is a chat line built by ChatReplyAction.
            BotQueuePacket<WorldPackets::Chat::ChatMessage>(session, delayedPacket.first);
        }
    });
...
#ifdef CMANGOS
   BotQueuePacket<WorldPackets::Spell::UseItem>(bot->GetSession(), *packet);
#endif
#ifdef MANGOS
...
#endif
    data << mInfo;
    bot->GetSession()->BotHandleMovementOpcodes(data);
