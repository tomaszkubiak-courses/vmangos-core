...
bool ServerFacade::IsSpellReady(Player* bot, uint32 spell, uint32 itemId)
{
    ...
    if (itemId)
    {
        ...
        return bot->IsSpellReady(sSpellMgr.GetSpellEntry(spell), proto);
    }
    else
        return bot->IsSpellReady(sSpellMgr.GetSpellEntry(spell));
#endif
}
...
#else
    // No IsFalling() here - the movement flags carry it. MOVEFLAG_JUMPING covers the
    // start of a fall, MOVEFLAG_FALLINGFAR the rest of it.
    return !unit->IsStopped() || unit->m_movementInfo.HasMovementFlag(MOVEFLAG_JUMPING | MOVEFLAG_FALLINGFAR);
#endif
#endif
}
