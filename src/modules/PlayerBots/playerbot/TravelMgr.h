...
namespace ai
{
	...
	private:
		...
		// Was boost::hash<std::pair<uint32, uint32>>. std::hash has no specialisation for
		// pair, so the key is folded by hand; the two halves are map ids, small and dense,
		// which a shift and xor separates well enough for this map.
		struct MapPairHash
		{
			std::size_t operator()(std::pair<uint32, uint32> const& key) const
			{
				return (static_cast<std::size_t>(key.first) << 32) ^ static_cast<std::size_t>(key.second);
			}
		};

		std::unordered_map<std::pair<uint32, uint32>, std::vector<MapTransfer>, MapPairHash> mapTransfersMap;
	};
}
