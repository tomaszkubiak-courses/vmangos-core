...
public:
    ...
    public:
        ...
        // cmangos's Map tracks which zones hold a player and the bots avoid the empty
        // ones. This core keeps no such record, and asking the map would count bots as
        // well - so answer it from the real players this manager already tracks.
        bool HasRealPlayerInZone(uint32 mapId, uint32 zoneId);
        bool HasRealPlayerOnMap(uint32 mapId);
        ...
        const std::vector<AuctionSnapshot>& GetAhPrices(uint32 itemId) {
            static const std::vector<AuctionSnapshot> emptyVector; // Avoid returning dangling refs
    ...
    protected:
        ...
        static void CitySpawnOnLogin(Player* bot);
    private:
    ...
    private:
        ...
        //                   itemId,             buyout, count
        std::unordered_map < uint32, std::vector<AuctionSnapshot>> ahMirror;
};
