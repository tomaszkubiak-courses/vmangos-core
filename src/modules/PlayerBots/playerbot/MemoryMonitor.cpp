...
void MemoryMonitor::Add(std::string objectType, uint64_t object, int level, std::string stack)
{
    ...
    {
        // Was boost::stacktrace. cpptrace is what VMaNGOS already vendors for this and
        // what Errors.cpp uses; with ENABLE_CPPTRACE=OFF there is no stack to record and
        // the entry keeps just its type and time.
        std::ostringstream out;
        if (stack.empty())
        {
#ifdef ENABLE_CPPTRACE
            out << cpptrace::generate_trace().to_string();
#else
            out << "<no stacktrace: built with ENABLE_CPPTRACE=OFF>";
#endif
        }
        else
            out << stack;
